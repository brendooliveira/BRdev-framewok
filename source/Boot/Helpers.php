...
{
    return "<link rel='stylesheet' href='".url("/shared/bootstrap/css/bootstrap.min.css")."'>";
}
...
{
    return "<script src='".url("/shared/bootstrap/js/bootstrap.min.js")."'></script>";
}

/**
 * @return string
 */
function bootstrapIcons(): string
{
    return "<link rel='stylesheet' href='".url("/shared/bootstrap-icons/bootstrap-icons.css")."'>";
}

//RANDOM

function numberRand(string $init, string $max): string
{
    return rand($init, $max);
}
...
function cache($cacheName, $nameArquivo) 
{
    if(!file_exists(__DIR__."/../../storage/caches/")){
        mkdir(__DIR__."/../../storage/caches/");
    }

    if(file_exists(__DIR__."/../../storage/caches/$nameArquivo")){
        require __DIR__."/../../storage/caches/$nameArquivo";
        $old = ob_get_clean(); 
        if($cacheName != $old){
            unlink(__DIR__."/../../storage/caches/$nameArquivo");
        }
    }

    if(!file_exists(__DIR__."/../../storage/caches/$nameArquivo")){
        ob_start();
        //ARQUIVO QUE SERÁ FEITO O CACHE
        echo $cacheName;
        $cache = ob_get_contents(); 

        ob_end_clean();
        file_put_contents(__DIR__."/../../storage/caches/$nameArquivo",$cache);
    }
...
{
    $dadosOne = base64_encode($data);
    return base64_encode($dadosOne);
}

function decode(string $data): string
{
    $dadosOne = base64_decode($data);
    return base64_decode($dadosOne);
}

function thumbS3(string $imagem): string
{
    return CONF_THUMB_AWS_S3."/$imagem";
}
