...
use Source\Support\Telegram\Bot;
...
class Web extends Controller
{
    ...
    public function home(): void
    {
        ...
        $cache = $this->view->render("home", [
            "head" => $head
        ]);

        //EXEMPLO PRA CACHE
        echo cache($cache,"home.html");
    }
    ...
    public function login(?array $data): void
    {
        ...
        if (!empty($data['csrf'])) {
            ...
            if ($login) {
                request_limit("weblogin", reset: true);
    ...
    public function reset(array $data): void
    {
        ...
        $hasCode = explode("|",$data["code"]);
        $email = $hasCode[0] && is_email($hasCode[0]) ? $hasCode[0] : null;
        $code = $hasCode[1] ?? null;

        if(!$email || !$code){
            $this->message->error("Link Invalido")->flash();
            redirect("/recuperar");
        }
    ...
    public function confirm(): void
    {
        ...
        echo $this->view->render("auth/optin", [
            ...
            "data" => (object)[
                ...
                "desc" => "Enviamos um link de confirmação para seu e-mail"
            ]
        ]);
    }
